...
var iotAgentLib = require('iotagent-node-lib'),
    ...
    context = {
        op: 'IOTAgent.LWM2MHandlers.Registration'
    };
...
function findConfiguration(device, callback) {
    ...
    iotAgentLib.getConfiguration(cleanResource, '', function(error, configuration) {
        ...
        } else if (device.type && commons.getConfig().iota.types[device.type]) {
            logger.error(context, 'peguei find config', commons.getConfig().iota.types);
            finalConfiguration = commons.getConfig().iota.types[device.type];
        } else if (device.type) {
...
function registrationHandler(endpoint, lifetime, version, binding, payload, callback) {
    ...
    function handleIncomingRegistrationRequest(error, lwm2mDevice, configuration) {
        if (error) {
            ...
            iotAgentLib.getDevice(endpoint,configuration.service, configuration.subservice, function(error, ngsiDevice) {
