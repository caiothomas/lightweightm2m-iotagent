...
var iotAgentLib = require('iotagent-node-lib'),
    ...
    apply = async.apply,
    context = {
        op: 'IOTAgent.LWM2MHandlers.UpdateRegistration'
    };
...
function updateRegistration(device, payload, callback) {
    ...
    function findService(device, callback){        
        registration.findConfiguration(device, function(error, device, configuration){
            var service;
            var subservice;
            
            if(configuration && !configuration.service && !configuration.subservice){  
                logger.debug(context, 'device', commons.getConfig().iota.service, commons.getConfig().iota.subservice);                
                callback(null, device.name, commons.getConfig().iota.service, commons.getConfig().iota.subservice);
            } else {
                callback(null, device.name, configuration.service, configuration.subservice);                
            }
        });
    }
    
    function teste(device, callback){
        logger.debug(context, 'device **', device);
    ...
    async.waterfall([
        ...
        apply(findService, device),
        apply(iotAgentLib.getDevice),
