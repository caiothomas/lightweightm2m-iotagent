...
var iotAgentLib = require('iotagent-node-lib'),
    ...
    iotAgentLwm2m = require('./iotAgentLwm2m'),
...
function extractOperations(device, attribute) {
    ...
    if (!operations && config.iota.types[device.type] &&
        config.iota.types[device.type].lwm2mResourceMapping &&
        config.iota.types[device.type].lwm2mResourceMapping[attribute]) {
        operations = config.iota.types[device.type].lwm2mResourceMapping[attribute].operations;
    }
...
function extractMapping(device, attribute) {
    ...
    } else if (config.iota.types[device.type] &&
        config.iota.types[device.type].lwm2mResourceMapping &&
        config.iota.types[device.type].lwm2mResourceMapping[attribute]) {
        mapping = config.iota.types[device.type].lwm2mResourceMapping[attribute];
...
function sendLwm2mOperation(operation, ngsiDevice, lwm2mDevice, attributeName, attributeValue, callback) {
    ...
    parameters.push(iotAgentLwm2m.getServer());    
...
function ngsiUpdateHandler(id, type, service, subservice, attributes, callback) {
    ...
    function updateDevice(ngsiDevice, lwm2mDevice, innerCallback) {
        ...
        console.log("vamos executar o updateDevice", ngsiDevice)
        console.log("lwm2",lwm2mDevice)
...
function ngsiExecuteHandler(id, type, service, subservice, attributes, callback) {
    var name = id.substring(0, id.indexOf(':'));
    console.log("Vamos executar EXE")
    ...
    function updateStatus(device, attributes, commandResult, statusCallback) {
        ...
        function updateAttributeStatus(attribute, innerCallback) {
            logger.debug(context, 'faz um update com o comando execudado', device.apikey, device.name, attribute, commandResult);
            console.log("updateStatus attributes", attributes)
            console.log("commandResult", commandResult)
            console.log("device", device.name, device.type, device.apikey)
...
function ngsiQueryHandler(id, type, service, subservice, attributes, callback) {
    ...
    function createContextElement(device, attributeValues, callback) {
        ...
        for (var i = 0; i < attributes.length; i++) {
            ...
            if (device.lazy) {
                lazySet = device.lazy;
            } else if (config.iota.types[type] && config.iota.types[type].lazy) {
                lazySet = config.iota.types[type].lazy;
            }
...
/*
    function readAttributes2(attributes, ngsiDevice, lwm2mDevice, innerCallback) {
        attributes = attributes.filter(function(attr) {
            return extractOperations(ngsiDevice, attr).indexOf('R') >= 0;
        });
        
        console.log("att", attributes)
        //attributes = ["1", "0", "2","3","4","5","6","2"]
        async.map(
            attributes,
            function(value, callback){                
                var mapping = extractMapping(lwm2mDevice, attributes);
                    if (!mapping) {
                        callback(new errors.OmaMappingNotFound());
                        return;
                    }
                lwm2mLib.read(lwm2mDevice.id, mapping.objectType, mapping.objectInstance, mapping.objectResource, iotAgentLwm2m.getServer(),  function (error, result) {
            if (error) {
            } else {
                console.log('\nResource read:\n----------------------------\n');
